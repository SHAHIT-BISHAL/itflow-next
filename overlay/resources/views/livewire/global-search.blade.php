            ...
            @if($results['clients']->isNotEmpty())
                <div class="border-b border-slate-100">
                    ...
                    @foreach($results['clients'] as $client)
                        <a href="{{ route('clients.show', $client) }}" wire:navigate data-search-result class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 text-sm text-slate-700">
            ...
            @if($results['tickets']->isNotEmpty())
                <div class="border-b border-slate-100">
                    ...
                    @foreach($results['tickets'] as $ticket)
                        <a href="{{ route('tickets.show', $ticket) }}" wire:navigate data-search-result class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 text-sm text-slate-700">
            ...
            @if($results['invoices']->isNotEmpty())
                <div class="border-b border-slate-100">
                    ...
                    @foreach($results['invoices'] as $invoice)
                        <a href="{{ route('invoices.show', $invoice) }}" wire:navigate data-search-result class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 text-sm text-slate-700">
            ...
            @if($results['deals']->isNotEmpty())
                <div>
                    ...
                    @foreach($results['deals'] as $deal)
                        <a href="{{ route('deals.show', $deal) }}" wire:navigate data-search-result class="flex items-center gap-2 px-4 py-2 hover:bg-slate-50 text-sm text-slate-700">
